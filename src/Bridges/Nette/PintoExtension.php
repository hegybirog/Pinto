...
class PintoExtension extends Nette\DI\CompilerExtension
{
	...
	public function loadConfiguration(): void
	{
		...
		$builder->addDefinition($this->prefix('logger'))
			->setClass(Pinto\ILogger::class)
			->setFactory([Pinto\Debugger::class, 'getLogger']);

		$builder->addDefinition($this->prefix('blueScreen'))
			->setFactory([Pinto\Debugger::class, 'getBlueScreen']);

		$builder->addDefinition($this->prefix('bar'))
			->setFactory([Pinto\Debugger::class, 'getBar']);
	}
	...
	public function afterCompile(Nette\PhpGenerator\ClassType $class): void
	{
		...
		if ($this->debugMode) {
			...
			if (
				!$this->cliMode
				&& Pinto\Debugger::getSessionStorage() instanceof Pinto\NativeSession
		...
		if (($dir = Pinto\Debugger::$logDirectory) && !is_writable($dir)) {
