...
class Panel
{
	constructor(id) {
		...
		this.elem.Pinto = this.elem.Pinto || {};
	}
	...
	init() {
		...
		this.init = function() {};
		elem.innerHTML = elem.dataset.PintoContent;
		Pinto.Dumper.init(Debug.layer);
		evalScripts(elem);
		...
		elem.addEventListener('mouseenter', () => {
			clearTimeout(elem.Pinto.displayTimeout);
		});
		...
		elem.querySelectorAll('.Pinto-icons a').forEach((link) => {
			link.addEventListener('click', (e) => {
				if (link.dataset.PintoAction === 'close') {
					this.toPeek();
				} else if (link.dataset.PintoAction === 'window') {
					this.toWindow();
				}
				e.preventDefault();
	...
	blur() {
		...
		if (this.is(Panel.PEEK)) {
			clearTimeout(elem.Pinto.displayTimeout);
			elem.Pinto.displayTimeout = setTimeout(() => {
	...
	restorePosition() {
		...
		} else if (this.elem.dataset.PintoContent) {
			this.init();
			this.toFloat();
			if (pos.resized) {
...
class Bar
{
	...
	close() {
		document.getElementById('Pinto-debug').style.display = 'none';
	}
...
function evalScripts(elem) {
	elem.querySelectorAll('script').forEach((script) => {
		if ((!script.hasAttribute('type') || script.type === 'text/javascript' || script.type === 'application/javascript') && !script.PintoEvaluated) {
...
Pinto.DebugPanel = Panel;
Pinto.DebugBar = Bar;
Pinto.Debug = Debug;
Pinto.getAjaxHeader = () => requestId + '_' + ajaxCounter++;
