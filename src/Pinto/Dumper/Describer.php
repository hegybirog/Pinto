...
final class Describer
{
	...
	private static function findLocation(): ?array
	{
		foreach (debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS) as $item) {
			if (isset($item['class']) && ($item['class'] === self::class || $item['class'] === Pinto\Dumper::class)) {
				$location = $item;
				continue;
				...
				try {
					...
					if (
						...
						|| preg_match('#\s@PintoSkipLocation\s#', (string) $reflection->getDocComment())
					) {
						$location = $item;
						continue;
					}
